# 목적: 업체 찾기/오퍼링 조회 (최저가 산정 시 옵션 제외, DB 경로 인자화, 만원 단위 입출력)
import sqlite3, json, argparse, os
from typing import List, Optional, Dict, Any

# 최저가 계산에서 제외할 패키지명(소문자 기준)
EXCLUDE_FROM_MIN = ("fitting_fee", "helper")
...
def main():
    ...
    if not os.path.exists(db_path):
        ...
        print("   - 현재 터미널 폴더와 DB 위치를 맞추거나, --db 로 절대경로를 지정하세요.")
        return
    ...
    for r in rows:
        print(
            ...
            f" | 최저가: {_fmt_price_won_to_manwon(r['min_price'])}"
            f" | 상품:{r['cnt_offers']}개"
        )
