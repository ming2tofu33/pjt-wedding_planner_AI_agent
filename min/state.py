...
class State(TypedDict, total=False):
    ...
    user_memo: Optional[UserMemo]
    memo_file_path: Optional[str]
    memo_needs_update: bool
    ...
    # 2) 라우팅 힌트/의사결정
    intent_hint: Optional[Literal["recommend", "tool", "general"]]
    ...
    rows: Annotated[List[Dict[str, Any]], add]
    ...
    # 6) 상태/에러/추적
    status: Literal["ok", "empty", "error", "handled_error"]
    reason: Optional[str]
    error_info: Optional[Dict[str, Any]]
    processing_timestamp: Optional[str]
    
    # LangGraph 대화 기록 (MessagesState 대체)
