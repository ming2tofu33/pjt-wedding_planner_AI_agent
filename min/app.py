    ...
    try:
        with conn.cursor() as cur:
            ...
            for table_name, type_name in table_map.items():
                # SQL 인젝션 방지를 위해 테이블 이름을 안전하게 처리합니다.
...
if 'chat_messages' not in st.session_state:
    st.session_state.chat_messages = [{"role": "assistant", "content": "안녕하세요! AI 웨딩 플래너 마리예요 ✨ 어떤 도움이 필요하신가요?"}]
    ...
    if prompt := st.chat_input("마리에게 물어보세요..."):
        ...
        with st.chat_message("assistant"):
            with st.spinner("마리가 생각 중이에요..."):
                time.sleep(1)
                responses = ['네, 그 부분 도와드릴게요! 예산과 선호도를 알려주시면 더 정확한 추천을 해드릴 수 있어요.', '좋은 선택이에요! 해당 업체의 상세 정보와 리뷰를 확인해보시겠어요?', '이런 점도 고려해보세요: 계약 조건, 취소 정책, 추가 비용 등을 꼼꼼히 확인하시는 것이 좋아요.', '현재 진행 상황을 체크해드릴게요. 다음 단계는 이렇게 진행하시면 됩니다!']
